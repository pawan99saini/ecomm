function imagePreview(input, img_id = '', img_err = '') {
    ...
    if (input.files && input_file) {
        ...
        let validExtensions = ["jpg" , "jpeg", "png", "bmp", "gif","pdf"];
        let reader = new FileReader();

        if ($.inArray(fextension, validExtensions) == -1) {
            $('#'+img_err).css('display', 'block').html('Only photo and pdf is allowed');
            input.value = "";
        ...
        if (fsize > 2097152) {
            $('#'+img_err).css('display', 'block').html('Photo and image should be less than 2mb');
           input.value = "";
            $('#'+img_id).css('display', 'block').attr('src', '');
           return false;
        }
