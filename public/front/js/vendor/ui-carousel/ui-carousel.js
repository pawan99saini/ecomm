        ...
        switch (f) {
          case "visible":
            if (!g || typeof g !== "number" || g <= 0 || Math.ceil(g) - g > 0) {
              throw new Error(
                "visible should be defined as a positive integer"
              );
            }
            break;
          case "step":
          ...
          case "speed":
            if (!g && g !== 0) {
              throw new Error(
                "speed should be defined as a number or a string"
              );
            }
            if (typeof g === "number" && g < 0) {
              ...
              if (
                typeof g === "string" &&
                !(g === "slow" || g === "normal" || g === "fast")
              ) {
                throw new Error(
                  'Only "slow", "normal" and "fast" values are valid'
                );
              }
            }
            break;
          case "navigation":
            if (!g || a.isPlainObject(g) === false) {
              throw new Error(
                "navigation should be defined as an object with at least one of the properties: 'prev' or 'next' in it"
              );
            }
            if (g.prev && typeof g.prev !== "string") {
              throw new Error(
                "navigation.prev should be defined as a string and point to '.class' or '#id' of an element"
              );
            }
            if (g.next && typeof g.next !== "string") {
              throw new Error(
                " navigation.next should be defined as a string and point to '.class' or '#id' of an element"
              );
            }
            break;
          case "auto":
            ...
            if (!(g.direction === "next" || g.direction === "prev")) {
              throw new Error(
                "direction: only 'right' and 'left' values are valid"
              );
            }
            if (
              isNaN(g.interval) ||
              ...
              g.interval < 0 ||
              Math.ceil(g.interval) - g.interval > 0
            ) {
      ...
      d = function (a, b, c, d, e, f) {
        function o() {
          (i = t() - g),
            p(),
            i >= m.time &&
              ((m.time = i > m.time ? i + h - (i - m.time) : i + h - 1),
              m.time < i + 1 && (m.time = i + 1)),
            m.time < d && (m.id = l(o));
        }
        function p() {
          d > 0
            ? ((m.currVal = s(m.time, j, n, d, e)),
              (k[b] = Math.round(m.currVal) + "px"))
            : (k[b] = c + "px");
        }
        function q() {
          (h = 1e3 / 60),
        ...
        function r() {
          null != m.id &&
            (window.requestAnimationFrame
              ? window.cancelAnimationFrame(m.id)
              : clearTimeout(m.id),
            (m.id = null));
        }
    ...
    return this.each(function () {
      ...
      e.data("imagePanning") ||
        (e
          ...
          .parent()
          .on("mousemove touchmove MSPointerMove pointermove", function (c, d) {
            var f = a(this);
            c.preventDefault();
            var h = f.height(),
              i = f.width(),
              ...
              l = k
                ...
                : c,
              m = [
                d ? ("center" === b ? h / 2 : 0) : l.pageY - f.offset().top,
                d ? ("center" === b ? i / 2 : 0) : l.pageX - f.offset().left,
              ];
            g = [
              Math.round((e.outerHeight(!0) - h) * (m[0] / h)),
              Math.round((e.outerWidth(!0) - i) * (m[1] / i)),
            ];
          })
          .find(".resize iframe")
          .each(function () {
