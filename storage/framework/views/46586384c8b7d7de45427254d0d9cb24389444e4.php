<script>
    $('body').on('click', '.pagination a', function(e) {
        ...
        if(mainId>0){
            ...
            $.get("<?php echo e(url('backoffice/product/sendimg')); ?>",{'id':mainId,page:page},function(n){
    ...
    $('.media-tabs').on('click',function(){
        ...
        if(id>0){
            ...
            if(content==""){
                $.get("<?php echo e(url('backoffice/product/sendimg')); ?>",{'id':id},function(n){
...
$(document).on('click',"#image-upload", function(){
    var drop = new Dropzone("#image-upload",{
        maxFilesize: 12,
        url: "<?php echo e(route('admin.media.popstore')); ?>",
                        ...
                        <div class="img-thumbnail thumbnail-imgess">
                            <input type="checkbox" id="myCheckbox${data.id}" data-id="${data.id}" data-img="${data.name}"/>
                              <label for="myCheckbox${data.id}" id="myLabel${data.id}">
                                <img onerror="handleError(this);"class="box-images px-2 py-2" image_id="" src="<?php echo e(asset("file")); ?>/${data.name}" alt="..." >
                            </label>
                        </div>
                    </div>`;
...
<?php /**PATH /home/daylist/public_html/resources/views/admin/mediascript/multiple_next.blade.php ENDPATH**/ ?>